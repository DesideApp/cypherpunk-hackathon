...
async function readOverviewArchiveHourly(rangeStart, rangeEnd) {
  ...
  for (let t = start.getTime(); t < end.getTime(); t += 3600000) {
    ...
    const keyGz = `overview/${yyyy}/${mm}/${dd}/${hh}.json.gz`;
    const keyJson = `overview/${yyyy}/${mm}/${dd}/${hh}.json`;
    ...
    try {
      ...
      try {
        ...
        const jsonBuffer = await readSnapshot({ key: keyJson, localPath: fileJson });
        if (!jsonBuffer) throw new Error('snapshot not found');
        snap = JSON.parse(jsonBuffer.toString('utf8'));
      }
...
async function readOverviewArchiveDaily(rangeStart, rangeEnd) {
  ...
  for (let t = start.getTime(); t < end.getTime(); t += 86400000) {
    ...
    const mm = String(d.getMonth()+1).padStart(2, '0');
    const dd = String(d.getDate()).padStart(2, '0');
    const keyGz = `overview-daily/${yyyy}/${mm}/${dd}.json.gz`;
    const keyJson = `overview-daily/${yyyy}/${mm}/${dd}.json`;
